...
export class RequestScheduler {
  ...
  //       axios(config).catch((error) => error),
  //     );
  //     console.log(batch);
  //     const results = await Promise.all(promises);

  //     // Resolve or reject the original promises
...
export class ReservationRpa {
  ...
  async start() {
    ...
    this.requestScheduler = new RequestScheduler(agent, 50, 500);
    Promise.all(chunks.map((chunk) => this.loginUser(chunk.data[0], chunk)));
